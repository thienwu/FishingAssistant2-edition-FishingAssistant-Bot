...
namespace ChibiKyu.StardewMods.FishingAssistant2.Frameworks
{
    internal class Assistant(Func<ModEntry> modEntry, Func<ModConfig> modConfig)
    {
        ...
        {
            if (_fishingRod == null) OverrideFishingRod(fishingRod);
            ...
            if (isModEnable && !Game1.player.isMoving()) CachePlayerPosition();
            else ForgetPlayerPosition();
        ...
        private void OverrideFishingRod(FishingRod fishingRod) => _fishingRod = new SFishingRod(fishingRod, modConfig);
        ...
        {
            if (_fishingRod == null || !_fishingRod.IsRodCanCast() || Game1.isFestival()) return;
            if (ModEntry.isBotHoldingCast) return; // Đang chạy lực ném
            ...
            // Kích hoạt trạng thái ném tự động
            ModEntry.isBotHoldingCast = true;
            ModEntry.botTargetCastPower = _fishingRod.SmartCastPowerSaved ? _fishingRod.SmartCastPower : modConfig().DefaultCastPower / 100.0f + 0.01f;
            
            Game1.pressUseToolButton();
            _fishingRod.Instance.castingPower = 0.0f; // Bắt buộc từ 0
            ...
            bool TryAutoEatFood()
            {
                ...
                {
                    if (item.Edibility <= 0 || (item.Category == Object.FishCategory && !modConfig().AllowEatingFish)) continue;
                    if (bestItem == null || bestItem.Edibility / bestItem.salePrice() < item.Edibility / item.salePrice()) bestItem = item;
                }
        ...
        {
            if (Game1.dayOfMonth != 1 || !ShouldGivePlayerFishingRod()) return;
            ...
            bool ShouldGivePlayerFishingRod() => !Game1.player.Items.OfType<FishingRod>().Any() && startWithFishingRod != "None";
        }

        internal void OnFishingMiniGameStart(BobberBar bobberBar)
        {
            IsInFishingMiniGame = true;
            _bobberBar = bobberBar;
        }
        ...
        private void DoPullFishFromWater()
        {
            ...
            if (bar is { fadeOut: true, scale: <= 0.1f })
            {
                ...
                // BẢN VÁ LỖI: Tính toán lại chính xác số lượng cá trước khi thực hiện pullFishFromWater
                _fishingRod.UpdateVanillaBaitMechanics(bar);

                if (bar.distanceFromCatching >= 0.8999f)
                {
                    rod.pullFishFromWater(bar.whichFish, bar.fishSize, bar.fishQuality, (int)bar.difficulty, bar.treasureCaught, bar.perfect, bar.fromFishPond, bar.setFlagOnCatch, bar.bossFish, rod.numberOfFishCaught);
                }
                else
                {
        ...
        private void DoTrashRecoveryMenu()
        {
            ...
            bool OnTrashCanReceiveRightClicked() => _inventoryPage != null && Game1.player.CursorSlotItem == null && Game1.input.GetMouseState().RightButton == ButtonState.Pressed && _inventoryPage.trashCan.containsPoint(Game1.getMouseX(true), Game1.getMouseY(true));
            void AddTrashToIgnoreList(Item item, Farmer who) { if (modConfig().JunkIgnoreList.Contains(item.QualifiedItemId)) return; modConfig().JunkIgnoreList.Add(item.QualifiedItemId); modEntry().Helper.WriteConfig(modConfig()); CommonHelper.PushWarning(item, I18n.HudMessage_AddToIgnoreList(), item.DisplayName); }
            void OnExit() { _itemsToTrash = trashRecoveryMenu.ItemsToGrabMenu.actualInventory.ToList(); _itemsToTrash.RemoveAll(trash => trash == null); }
        }
        ...
        private void AutoCloseFishPopup()
        {
            ...
            if (_autoClosePopupDelay-- > 0) return;
            ...
            {
                foreach (InputButton button in Game1.options.useToolButton) { if (button.key != Keys.None) Game1.oldKBState = new KeyboardState(button.key); }
                callback?.Invoke();
        ...
        private void AutoLootTreasure()
        {
            ...
            if (actualInventory.Count == _excludeItems.Count)
            {
                ...
                {
                    if (modConfig().ActionIfInventoryFull == ActionOnInventoryFull.Drop.ToString()) { _treasureChestMenu.DropRemainingItems(); _treasureChestMenu.exitThisMenu(); }
            ...
            else
            {
                ...
                if (item.QualifiedItemId == "(O)102") { Game1.player.foundArtifact(item.QualifiedItemId, 1); Game1.playSound("fireball"); actualInventory.RemoveAt(_excludeItems.Count); }
                else
                {
                    if (Game1.player.addItemToInventoryBool(item)) { Game1.playSound("coin"); actualInventory.RemoveAt(_excludeItems.Count); }
                    else _excludeItems.Add(item);
                }
                _autoLootDelay = 10;
            }
        }
        ...
        private void AutoPauseFishing(string autoPauseFishing, int numToWarn)
        {
            ...
            modEntry().ForceDisable();
            if (!Game1.IsMultiplayer && !modEntry().SavedRoutes.ContainsKey("AutoFishing")) Game1.activeClickableMenu = new GameMenu();
        }
        ...
        internal void AutoTrashJunk(InventoryChangedEventArgs e)
        {
            ...
            bool CanTrashThisItem(Item itemToTrash) => itemToTrash.canBeTrashed() && !modConfig().JunkIgnoreList.Contains(itemToTrash.QualifiedItemId);
            bool TrashConditionsMet(Item itemToTrash)
            {
                int sellToStorePrice = Utility.getSellToStorePriceOfItem(itemToTrash, false);
                return itemToTrash.Category == Object.junkCategory || (sellToStorePrice <= modConfig().JunkHighestPrice && ((itemToTrash.Category == Object.FishCategory && modConfig().AllowTrashFish) || (itemToTrash.Category != Object.FishCategory && SaveToTrashed(itemToTrash))));
            }
            bool SaveToTrashed(Item itemToTrash) => itemToTrash is not (MeleeWeapon or FishingRod or Pan or Slingshot);
        }
        ...
        {
            if (toggle) CachePlayerPosition();
            else ForgetPlayerPosition();
            _fishingRod?.ResetSmartCastPower();
        }

        internal void OnGameMenuOpen(GameMenu gameMenu) => _inventoryPage = gameMenu.pages[0] as InventoryPage;
        internal void OnGameMenuClose() => _inventoryPage = null;
        internal void OnTreasureMenuOpen(ItemGrabMenu itemGrabMenu) { _treasureChestMenu = itemGrabMenu; _isInTreasureChestMenu = true; }
        internal void OnTreasureMenuClose() { _isInTreasureChestMenu = false; _treasureChestMenu = null; }
        
        public void OnConfigSaved() => _fishingRod?.ResetSmartCastPower();
    }
}